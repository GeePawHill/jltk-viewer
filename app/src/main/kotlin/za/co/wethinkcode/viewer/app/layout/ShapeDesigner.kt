...
import za.co.wethinkcode.viewer.app.parse.*
...
class ShapeDesigner {
    ...
    private fun commitShape(commit: Commit, previousUpperRight: LogPoint, paths: LogShapes): LogPoint {
        val upperRightFromRuns = runsForCommit(previousUpperRight, commit, paths)
        paths.shapes.add(CommitShape(commit, LogPoint(previousUpperRight.x, upperRightFromRuns.y)))
    ...
    private fun runShape(run: LogDetail, previousUpperRight: LogPoint, paths: LogShapes): LogPoint {
        when (run.type) {
    ...
    private fun makeUnknownShape(run: LogDetail, previousUpperRight: LogPoint, paths: LogShapes): LogPoint {
        paths.shapes.add(BarShape(run, previousUpperRight, Color.LIGHTBLUE, Color.BLACK))
        return LogPoint(previousUpperRight.x + 1, previousUpperRight.y)
    }

    private fun makeBase64Shape(run: LogDetail, previousUpperRight: LogPoint, paths: LogShapes): LogPoint {
        paths.shapes.add(BarShape(run, previousUpperRight, Color.YELLOW, Color.BLACK))
        return LogPoint(previousUpperRight.x + 1, previousUpperRight.y)
    }

    private fun makeRunShape(run: LogDetail, previousUpperRight: LogPoint, paths: LogShapes): LogPoint {
        paths.shapes.add(BarShape(run, previousUpperRight, Color.DARKGRAY, Color.DARKGRAY))
        return LogPoint(previousUpperRight.x + 1, previousUpperRight.y)
    }

    private fun makeTestShape(detail: LogDetail, previousUpperRight: LogPoint, paths: LogShapes): LogPoint {
